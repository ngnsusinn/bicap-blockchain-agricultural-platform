...
BICAP được triển khai theo mô hình **monorepo** gồm 3 thành phần:

| Thành phần | Thư mục | Công nghệ | Vai trò |
|---|---|---|---|
| Backend API | `backend/` | Java 21, Spring Boot 3.3 | REST API, nghiệp vụ, tích hợp blockchain; phục vụ static bundle ở chế độ một port |
| Web App (Portal + Admin) | `web/` | React 19, TypeScript, Vite 8 | Một app React duy nhất: portal Farm/Retailer/Shipping/Guest tại `/`, dashboard quản trị tại `/admin` |
| Smart Contracts & Dev tools | `dev/` | Solidity 0.8.24, OpenZeppelin Upgradeable | `dev/blockchain/contracts/Traceability.sol` (UUPS, AccessControl, Pausable, ReentrancyGuard) cùng `dev/loadtest/`, `dev/tools/` |

> **Sau refactor:** `mobile-app/` và toàn bộ Docker (`Dockerfile`, `docker-compose.db.yml`, `web/Dockerfile`, `web/nginx.conf`) đã được gỡ khỏi repo; job Docker trong CI cũng bị xoá. Backend **vẫn giữ** `DriverMobileController` + `/api/driver/**` (actor Shipping Driver). Ports chỉ còn **8080** (backend) và **5174** (web dev server).
...
- Trang chủ (portal) có 3 tab đăng nhập Farm/Retailer/**Admin** + nút **điền nhanh tài khoản test**; đăng nhập ADMIN tự chuyển hướng sang `/admin/?token=…` — AdminApp đổi token qua `/api/profile` và vào thẳng dashboard (SSO một port).
...
cd web && npm ci && npm run build                    # build app React → web/dist
cp -r dist/* ../backend/src/main/resources/static/   # copy static bundle vào backend
cd ../backend && mvn spring-boot:run                 # → http://localhost:8080
```
JAR chứa sẵn bundle: portal tại `/`, Admin dashboard tại `/admin/`.
