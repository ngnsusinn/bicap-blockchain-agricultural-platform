...
describe('auth session helpers', () => {
  ...
  it('logout clears every credential, including legacy admin-web keys', () => {
    saveSession('tok', { id: 1, email: 'a@b.c', fullName: 'A', role: 'RETAILER' }, 'rt');
    localStorage.setItem('bicap_session', 'legacy');
    localStorage.setItem('bicap_token', 'legacy');
    logout();
    expect(isLoggedIn()).toBe(false);
    expect(localStorage.getItem('currentUser')).toBeNull();
    expect(localStorage.getItem('refreshToken')).toBeNull();
    expect(localStorage.getItem('bicap_session')).toBeNull();
    expect(localStorage.getItem('bicap_token')).toBeNull();
  });
